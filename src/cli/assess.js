 ...
 */
async function printCreativeOutput(rule, profile, move, options = {}) {
  ...
  const result = await generateCreativeOutput(rule.redirectAction, profile, move, { forceMock });
  
  if (result.success) {
    // Show source indicator
    let sourceIndicator;
    if (result.source === 'granite') {
      sourceIndicator = colorize('  [Generated by IBM Granite 3.3-8B via Replicate]', colors.dim + colors.green);
      if (result.predictionId) {
        console.log(sourceIndicator);
        console.log(colorize(`  Prediction ID: ${result.predictionId}`, colors.dim + colors.green));
      } else {
        console.log(sourceIndicator);
      }
    } else {
      sourceIndicator = colorize('  [MOCK OUTPUT — no API token / API unavailable]', colors.bright + colors.yellow);
      console.log(sourceIndicator);
    }
    console.log('');
...
export async function runAssessment(profile, move, options = {}) {
  const { verbose = true, forceMock = false } = options;
  ...
  if (assessment.triggeredRules.length > 0) {
    ...
    for (let index = 0; index < assessment.triggeredRules.length; index++) {
      ...
      if (rule.severity === 'block') {
        await printCreativeOutput(rule, profile, move, { forceMock });
      }
    }
  } else {
...
async function main() {
  ...
  if (filteredArgs.length === 0) {
    console.log(colorize('Usage: node assess.js <scenario> [--mock]', colors.yellow));
    console.log('\nOptions:');
    console.log('  --mock    Force mock generator (ignore REPLICATE_API_TOKEN)');
    console.log('\nAvailable scenarios:');
  ...
  import('../../tests/fixtures/sample-profiles.js').then(async (fixtures) => {
    ...
    await runAssessment(profile, move, { forceMock });
  }).catch(error => {
