 ...
 */
export function runAssessment(profile, move, options = {}) {
  ...
  if (assessment.triggeredRules.length > 0) {
    ...
    assessment.triggeredRules.forEach((rule, index) => {
...
if (import.meta.url === `file://${process.argv[1]}`) {
  ...
  import('../../tests/fixtures/sample-profiles.js').then(fixtures => {
    let profile, move;
